...
def epoch_train(gpu, model, optimizer, batch_size, component,embed_layer,data, prepared_tables, table_type, use_tqdm, bert_model):
    ...
    for _ in tqdm.tqdm(range(len(data) // batch_size), disable=not use_tqdm):
        ...
        if component == "multi_sql":
            ...
            q_emb, q_len, q_nl_len, table_sep_locs, selected_tables = embed_layer.gen_bert_batch_with_table(q_seq, tabs, cols, foreign_keys)
            new_history = []
            ...
            score = model.forward(q_emb, q_len, q_nl_len, hs_emb_var, hs_len, table_sep_locs, selected_tables)
            new_labels = []
            for b, tables in enumerate(selected_tables):
                one_new_label = []
                for table in tables:
                    if str(table) not in label[b]:
                        one_new_label.append(0.)
                    else:
                        one_new_label.append(1.)
                new_labels.append(one_new_label)
...
def from_acc(model, embed_layer, data, max_batch):
    ...
    for datum in tqdm.tqdm(data):
        ...
        q_emb, q_len, q_nl_len, table_sep_locs, selected_tables = embed_layer.gen_bert_for_eval(one_q_seq, one_tab_names, one_cols, foreign_keys)
        st = 0
        b = len(q_emb)
        scores = []
        while st < b:
            ...
            hs_emb_var, hs_len = embed_layer.gen_x_history_batch(history)
            score = model.forward(q_emb[st:ed], q_len[st:ed], q_nl_len[st:ed], hs_emb_var, hs_len, table_sep_locs[st:ed], selected_tables[st:ed])
