...
class WordEmbedding(nn.Module):
    ...
    def gen_bert_batch_with_table(self, q, tables, table_cols, foreign_keys):
        ...
        for idx, one_q in enumerate(q):
            ...
            generated_tables = generate_random_two_table(len(tables[idx]))
            ...
            for token_idx, token in enumerate(tokenozed_one_q):
                if token == "[SEP]":
                    if token_sep_found in table_sep_nums:
                        one_table_sep_locs.append(token_idx)
                        table_seeing = True
                    else:
                        table_seeing = False
                    token_sep_found += 1
                    ...
                    if table_seeing:
                        one_table_locs.append(token_idx)
            table_locs.append(one_table_locs)
            table_sep_locs.append(one_table_sep_locs)
        ...
        if self.gpu:
            tokenized_q = tokenized_q.cuda()
        return tokenized_q, q_len, q_nl_len, table_sep_locs, selected_tables
    ...
    def gen_bert_for_eval(self, one_q, one_tables, one_cols, foreign_keys):
        ...
        table_lists = []
        for tab1 in range(len(one_tables)):
            for tab2 in range(tab1 + 1, len(one_tables)):
                table_lists.append([tab1, tab2])
        ...
        for b in range(B):
            ...
            current_sep_num = 0
            q_nl_len.append(len(self.bert_tokenizer.convert_tokens_to_ids(self.bert_tokenizer.tokenize(input_q))))
            for tab in table_lists[b]:
                table_name = one_tables[tab]
                input_q += " [SEP] " + table_name
                table_sep_nums.append(current_sep_num)
                current_sep_num += 1
        ...
        if self.gpu:
            tokenized_q = tokenized_q.cuda()
        return tokenized_q, q_len, q_nl_len, table_sep_locs, table_lists
